(function ($, wc_stripe) {
    ...
    LocalPayment.prototype.process_setup_intent = function (data) {
        // create the setup intent
        this.block();
        this.create_setup_intent(data).then(function (response) {
            this.stripe[this.setupActionMethod](this.client_secret, this.get_confirmation_args(response)).then(function (result) {
                if (result.error) {
    ...
    LocalPayment.prototype.get_error_from_result = function (result) {
        if (result.error) {
            return result.error;
        }
        if (result.paymentIntent && result.paymentIntent.last_payment_error) {
            return result.paymentIntent && result.paymentIntent.last_payment_error;
        }
        return null;
    }
    ...
    LocalPayment.prototype.processConfirmation = function (obj) {
        if (obj.type === 'payment_intent') {
            this.stripe[this.confirmation_method](obj.client_secret, this.get_confirmation_args(obj)).then(function (result) {
                if (result.error || (result.paymentIntent && result.paymentIntent.last_payment_error)) {
    ...
    LocalPayment.prototype.get_confirmation_args = function (obj) {
        ...
        if (obj.confirmation_args && Object.keys(obj.confirmation_args).length) {
            args = $.extend({}, args, obj.confirmation_args);
        }
        if (!args.payment_method.billing_details) {
            args.payment_method.billing_details = obj.billing_details;
        }
        return args;
    }
    ...
    function PromptPay(params) {
        this.confirmation_method = 'confirmPromptPayPayment';
        LocalPayment.call(this, params);
        window.addEventListener('hashchange', this.hashChange.bind(this));
    }
    ...
    BLIK.prototype.after_confirmation_error = function (result) {
        clearInterval(this.timer_id);
        $('.blik-timer-container').hide();
        $('.wc-stripe-blik-code-container').show();
    }
    ...
    Konbini.prototype.get_confirmation_args = function (obj) {
        var args = LocalPayment.prototype.get_confirmation_args.apply(this, arguments);
        if (this.generateConfirmationNumber) {
            args.payment_method_options.konbini.confirmation_number = obj.confirmation_number;
        }
        return args;
    }
